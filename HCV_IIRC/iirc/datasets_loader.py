...
datasets_names = ["incremental_cifar100", "iirc_cifar100","my_iirc_cifar100", "incremental_imagenet_full", "incremental_imagenet_lite",
                  "iirc_imagenet_full", "iirc_imagenet_lite", "iirc_imagenet_subset", "isic_2019"]
    ...
    if dataset_name == "iirc_cifar100":
        ...
        datasets = \
            _load_incremental_imagenet(root=dataset_root,
                                       intask_valid_train_ratio=dataset_configuration["intask_valid_train_ratio"],
                                       posttask_valid_train_ratio=dataset_configuration["posttask_valid_train_ratio"])
    elif dataset_name == "isic_2019":
        dataset_configuration = dataset_configuration["isic_2019"]

        datasets = \
            _load_isic_2019(root=dataset_root,
                                intask_valid_train_ratio=dataset_configuration["intask_valid_train_ratio"],
                                posttask_valid_train_ratio=dataset_configuration["posttask_valid_train_ratio"])
...
def _load_isic_2019(root: str = ".", intask_valid_train_ratio: float = 0.1,
                        posttask_valid_train_ratio: float = 0.1) -> Dict[str, DatasetStructType]:
    """
    ...
    for ind  in range(len(X_train)):
        ...
        image = cv2.resize(image, (64, 64), cv2.INTER_AREA) # remove later
    ...
    for subclass in subclasses_to_superclass.keys():
        ...
        for dataset_type in datasets.keys():
            if dataset_type == "test":
                ...
                raw_data = raw_data_train
            for idx in samples_idx[dataset_type]:
                image = raw_data[idx][0].reshape((3, 64, 64)).transpose(1, 2, 0)
                image = Image.fromarray(image)
                if superclass is None:
                    datasets[dataset_type].append((image, (subclass,)))
                else:
...
def _get_tasks_configuration(dataset_name: str, tasks_configuration_id: int = 0, joint: bool = False) -> Tuple[
    ...
    #################################################
        ...
        tasks_file = os.path.join(metadata_root, "isic_2019_task_configurations.json")
